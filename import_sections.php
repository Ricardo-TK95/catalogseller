...
if ($step === 'upload' && $_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!isset($_POST[$csrf_name]) || !hash_equals($_SESSION[$csrf_name], (string)$_POST[$csrf_name])) { $erro='Session expired. Please reload.'; }
    ...
    if (!isset($_FILES['csv']) || $_FILES['csv']['error'] !== UPLOAD_ERR_OK) { $erro='Please select a CSV/XLSX file.'; }
      ...
      if(!$erro){ $tmp = sys_get_temp_dir().DIRECTORY_SEPARATOR.'import_sections_'.bin2hex(random_bytes(6)).'.'.$ext; if(!move_uploaded_file($_FILES['csv']['tmp_name'],$tmp)){ $erro='Failed to store uploaded file.'; } else { $tmpfile=$tmp; $step='map'; }}
    }
  }
}
  ...
  if (!isset($_POST[$csrf_name]) || !hash_equals($_SESSION[$csrf_name], (string)$_POST[$csrf_name])) { $erro='Session expired. Please reload.'; }
    ...
    if (!$erro) {
      ...
      $push = function($desc) use (&$to_insert, &$seen){
        ...
        if ($desc==='') return;
        $key = mb_strtoupper($desc);
        if (isset($seen[$key])) return; // distinct within file
        $seen[$key] = true;
        $to_insert[] = ['description'=>$desc,'display_order'=>100,'is_active'=>1];
      };
      $ext=strtolower(pathinfo($tmpfile, PATHINFO_EXTENSION));
      if($ext==='xlsx'){
        $all = read_xlsx_rows($tmpfile, 0, $sheetpath ?: 'xl/worksheets/sheet1.xml');
        $start = $has_header ? 1 : 0;
        for ($i=$start;$i<count($all);$i++) { $row=$all[$i]; $desc = $row[$map_desc] ?? ''; $push($desc); }
      } else {
      ...
      if (!$erro && !empty($to_insert)) {
        // Load existing descriptions to avoid duplicates
        $existing = [];
        if ($stmt=$conn->prepare('SELECT description FROM sections WHERE company_id=?')){ $stmt->bind_param('i',$company_id); if($stmt->execute()){ $res=$stmt->get_result(); while($r=$res->fetch_assoc()){ $existing[mb_strtoupper(trim($r['description']))]=true; } $res->free(); } $stmt->close(); }
        ...
        try {
          ...
          foreach($to_insert as $s){
            $key=mb_strtoupper(trim($s['description']));
            if(isset($existing[$key])) { $skipped_existing++; continue; }
            $stmt->bind_param('isii',$company_id,$s['description'],$s['is_active'],$s['display_order']);
            if(!$stmt->execute()) throw new Exception('Insert failed');
            $existing[$key]=true; $inserted++;
          }
          $stmt->close(); $conn->commit();
          if ($inserted>0) { $ok='Imported '.$inserted.' section(s). Some duplicates/existing may have been skipped.'; }
          else { $ok='No new sections were inserted. All rows were empty or already existed.'; }
...
<body>
  <div class="container">
    <div><a class="btn" href="sections.php">Back</a></div>
    <section class="card">
      <h2 style="margin:0 0 10px 0">Import Sections</h2>
      ...
      <p class="muted" style="margin-top:8px">Required: Description. Optional: Display order (default 100). Active = Yes.</p>
      <?php elseif ($step === 'map'): ?>
      <form method="post" class="grid">
        ...
        <div class="col-12">
          ...
          <a class="btn" href="import_sections.php">Cancel</a>
        </div>
      </form>
      <?php endif; ?>
    </section>
